...
function transformTrailblazerStats(record) {
  ...
  return {
    ...
    shadowK:    record.Points_K__c, // safe mapping
...
function transformArticles(records) {
  return records.map(r => ({
    title:         r.Title,
    summary:       r.Summary,
    urlName:       r.UrlName,
    articleNumber: r.ArticleNumber,
    publishedDate: r.LastPublishedDate ? r.LastPublishedDate.split("T")[0] : "",
  }));
}
...
async function main() {
  ...
  let articles = [];
  try {
    articles = await soql(token,
      `SELECT Title, Summary, UrlName, ArticleNumber, LastPublishedDate
       FROM Knowledge__kav
       WHERE PublishStatus = 'Online' AND Language = 'en_US'
       ORDER BY LastPublishedDate DESC`
    );
  } catch (e) {
    console.warn("⚠️  Failed to query Knowledge__kav (Salesforce Knowledge might not be enabled in this org):", e.message);
  }
